...
| Ledger/Developer Mode | Developer Mode is separate from the Privacy Access Ledger and MUST NOT emit, record, or retain raw protected/persona/host values; it uses only bounded, redacted, allowlisted metadata/categories/status evidence, MUST NOT weaken mediation, and MUST NOT promote missing evidence to success. | Separation, privilege, redaction, and failure tests | Unknown — not yet evidenced |
| Repository/publication safety (PD-REQ-085) | Repository content, PRs, retained/uploaded CI artifacts, Actions/project-review logs, and diagnostic evidence intended for project review are treated as public regardless of visibility and contain no secrets, credentials, private user data, raw protected/persona/host values, or production signing material. | Diff/tree review, secret scanning where applicable, log/artifact inspection, and diagnostic redaction tests | Unknown — not yet evidenced |
